 ...
 *  chartData: { day: string, minutes: number, status: 'green'|'yellow'|'red' }[],
    ...
    <section className="est-card est-insights">
      <div className="est-insights__grid">
        <article className="est-metric">
          <p className="est-metric__label">Daily goal</p>
          <p className="est-metric__value">{dailyGoal} min</p>
          <p className="est-muted">Today: {todayMinutes} min</p>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={chartData} margin={{ top: 4, right: 8, left: -22, bottom: 0 }}>
              ...
              <Bar dataKey="minutes" radius={[6, 6, 0, 0]}>
                {chartData.map((entry) => (
                  <Cell key={entry.day} fill={`var(--est-${entry.status})`} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
